When /^I validate retail items as an employee$/ do
  ...
  if @count == 1
    ...
    @browser.goto("#{@server}/employee/admin/frm_orderchoose.aspx?OT=CRT")
  ...
  while @rows < 1200
    ...
    if @count == 2
      ...
      if @browser.text.include?('That item or description that you entered could not be found in our database and cannot be added to your order') or @browser.text.include?('The item that you entered is not allowed for this type of order')
        ...
        puts "SKU #{@productid} Could not be added to a retail order through employee back office in the US."
